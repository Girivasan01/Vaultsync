...
export default function DashboardPage() {
  ...
  const { stats, storage, activity, recent, loading } = useDashboard(activeOrgId, activeEnterpriseId);

  if (loading) return <Skeleton className="h-96 w-full" />;

  const cards = [
    ['Total Applications', stats.totalApplications],
    ['Total Enterprises', stats.totalEnterprises ?? 0],
  ...
  return (
    <div className="space-y-6">
      {(activeOrgName || activeEnterpriseName) && (
        <p className="text-sm font-medium text-gray-500">
          Showing data for:
          {activeOrgName ? <span className="text-ocean"> {activeOrgName}</span> : null}
          {activeEnterpriseName ? <span className="text-ocean"> / {activeEnterpriseName}</span> : null}
        </p>
      )}
      <div className="grid gap-4 md:grid-cols-4">
        {cards.map(([label, value]) => (
          <Card key={label}><p className="text-sm text-gray-500">{label}</p><p className="mt-2 text-2xl font-bold">{value}</p></Card>
        ))}
      </div>
            ...
            <tbody>
              {recent.map((backup) => (
                <tr key={backup.id} className="border-b dark:border-gray-800">
                  ...
                  <td>{formatDistanceToNow(new Date(backup.createdAt), { addSuffix: true })}</td>
                </tr>
              ))}
            </tbody>
          </table>
